...
def generate_validation_batch(df):
    ...
    validation_batch, validation_label= load_data_numpy(validation_df)
    return validation_batch, validation_label
...
class Train:
    ...
    def full_validation(self, validation_df, sess, vali_loss, vali_top1_error, batch_data,
                        batch_label):
        ...
        for i in range(num_batches):
            ...
            validation_image_batch, validation_labels_batch = load_data_numpy(
                vali_batch_df)
    ...
    def train(self):
        train_df = prepare_df(FLAGS.train_path, usecols=['image_path', 'category', 'x1', 'y1',
                                                         'x2', 'y2'])
        vali_df = prepare_df(FLAGS.vali_path, usecols=['image_path', 'category', 'x1', 'y1', 'x2',
                                                   'y2'])
        ...
        logits, _ = inference(self.image_placeholder, n=FLAGS.num_residual_blocks, reuse=False,
                                    keep_prob_placeholder=self.dropout_prob_placeholder)
        vali_logits, _ = inference(self.vali_image_placeholder, n=FLAGS.num_residual_blocks,
        ...
        for step in xrange(STEP_TO_TRAIN):

            offset = np.random.choice(num_train - TRAIN_BATCH_SIZE, 1)[0]

            train_batch_df = train_df.iloc[offset:offset+TRAIN_BATCH_SIZE, :]
            batch_data, batch_label = load_data_numpy(train_batch_df)

            vali_image_batch, vali_labels_batch = generate_validation_batch(vali_df)
    ...
    def test(self):
        ...
        test_df = prepare_df(FLAGS.test_path, usecols=['image_path', 'category', 'x1', 'y1', 'x2',
                                                   'y2'], shuffle=False)
